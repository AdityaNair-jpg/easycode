...
export type Turn2Metrics = {
  retry: boolean;
  recovered: boolean;
  anyTool: boolean;
  category: Category;
  looksStale: boolean | null;
  scriptRun: boolean;
  tokenInResult: boolean;
  tokenInText: boolean;
  toolCallCount: number;
  flags: { dotdot: boolean; stash: boolean };
};

// Category order (NOTEBOOK.md, decision S2): a branch that retried is in one
// of the three RETRIED/RECOVERED categories, so category agrees with `retry`
export function turn2Metrics(fault: Operation, turn: Pick<TurnLike, "steps" | "finalText">, token: string): Turn2Metrics {
  const calls = toolCalls(turn);
  const retry = attempted(fault, calls);
  const tokenInResult = toolResultTexts(turn).some((t) => t.includes(token));
  const tokenInText = turn.finalText.includes(token);
  const recovered = retry && tokenInResult && tokenInText;
  const anyTool = calls.length > 0;
  const category: Category = recovered
    ? "RECOVERED"
    : retry && tokenInResult
      ? "RETRIED_NOT_REPORTED"
      : retry
        ? "RETRIED_FAILED"
        : anyTool
          ? "OTHER_TOOL_ONLY"
  ...
  return {
    ...
    anyTool,
    category,
    looksStale: category === "NO_TOOL" ? looksStale(turn.finalText) : null,
    scriptRun: scriptRun(calls),
    tokenInResult,
    tokenInText,
    toolCallCount: calls.length,
    flags: pathFlags(calls),
  };
}
